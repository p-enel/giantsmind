...
from giantsmind.get_metadata import get_metadata, deal_with_missing_fields
from giantsmind.utils import get_local_data_path, set_env_vars
...
def save_payloads_to_json(
    ...
    folder_path = Path(get_local_data_path()) / "parsed_docs"
...
def update_metadata_df(metadatas: List[dict], hashes: List[str], pdf_files: List[str]) -> None:
    """Update the metadata dataframe with new metadata."""
    metadatas = deepcopy(metadatas)
    new_metadatas = []
    for metadata, hash, file_path in zip(metadatas, hashes, pdf_files):
        metadata.update({"hash": hash, "file_path": file_path})
        new_metadatas.append(metadata)

    metadata_df = load_metadata_df()

    metadata_df = pd.concat((metadata_df, pd.DataFrame(new_metadatas).set_index("hash")))
    metadata_df = metadata_df[~metadata_df.index.duplicated(keep="first")]
    save_metadata_df(metadata_df)
...
def parse_files(files: Sequence[str | Path], instruction: str) -> list[str]:
    """Parse a list of files using Llama."""
    if len(files) == 0:
        print("No files to parse.")
        return []

    output_folder = Path(get_local_data_path()) / "parsed_docs"
...
def chunk_documents(
    documents: Sequence[str], chunk_size: int = 4096, chunk_overlap: int = 256
) -> List[List[Document]]:
...
def setup_database_and_collection(collection_name: str, embeddings_model: str) -> QdrantClient:
...
def process_metadata(files: Sequence[str], hashes: Sequence[str], verbose: bool = True) -> None:
    ...
    metadatas = [deal_with_missing_fields(metadata, pdf_path) for metadata, pdf_path in zip(metadatas, files)]
    payloads = [
...
if __name__ == "__main__":
    ...
    unproc_hashes, unproc_files = get_unprocessed_pdf_files(client, collection, pdf_folder)
